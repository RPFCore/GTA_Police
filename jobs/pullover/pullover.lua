...
RegisterNetEvent('rpf:mimic')
AddEventHandler('rpf:mimic', function()
	if stopped then
		--mimick
		...
		if mimicking then
			...
			ShowNotification( "Vous devez arreter le civile avant." )
		end
	end
end, false)
...
AddEventHandler('po:mimic', function()
	...
	if DoesEntityExist(stoppedVeh) then
		ShowNotification("La ~r~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(stoppedVeh))) .. " ~w~vous suit a present.")
		mimicking = true
...
AddEventHandler('po:unmimic', function()
	mimicking = false
	Citizen.Wait(100)
	ShowNotification("La ~r~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(stoppedVeh))) .. " ~w~ne vous suite plus a present.")
	stopped = true
	lockedin = false
	Wait(2000)
...
RegisterNetEvent('rpf:follow')
AddEventHandler('rpf:follow', function()
	if stopped then
		--follow
		...
		if following then
			...
			--(You need to pull over a vehicle first)
			ShowNotification( "Vous devez arreter le vehicule avant." )
		end
	end
end, false)
...
AddEventHandler('po:follow', function()
	...
	local playerVeh = GetVehiclePedIsIn(player, false)
		if DoesEntityExist(stoppedVeh) then
			ShowNotification("La ~r~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(stoppedVeh))) .. " ~w~vous suit a présent.")
			following = true
			Citizen.Wait(10)
			while (following) do 
				Citizen.Wait(1000)
...
AddEventHandler('po:unfollow', function()
	following = false
	Citizen.Wait(100)
	ShowNotification("La ~r~" .. GetLabelText(GetDisplayNameFromVehicleModel(GetEntityModel(stoppedVeh))) .. " ~w~ne vous suit plus a présent.")
	stopped = true
	lockedin = false
	Wait(2000)
	SetPedIntoVehicle(stoppedDriver, stoppedVeh, -1)
...
RegisterNetEvent('rpf:ticket')
AddEventHandler('rpf:ticket', function()
	local ticket = "CODE_HUMAN_MEDIC_TIME_OF_DEATH"
	ShowNotification("~o~Officier :~w~ " .. "Voici votre amande d'un montent de ~g~" .. price .. " ~w~pour ~y~" .. reason)
	...
	if driverAttitude < 50 then
		DriverTicketQuotes = {"D'accord...","Je comprend.","Ok, tout est bon."}
	else
		DriverTicketQuotes = {"Oh aller quoi !","Vous me degouter.","Tous le mode deteste la police.","Va arreter les vrait criminel."}
	end
		ShowNotification("~o~Chauffeur :~w~ " .. DriverTicketQuotes[math.random(#DriverTicketQuotes)])
end)

RegisterNetEvent('rpf:getplate')
AddEventHandler('rpf:getplate', function()
	vehPlateNum = GetVehicleNumberPlateText(targetVeh)
end)

RegisterNetEvent('rpf:runplate')
AddEventHandler('rpf:runplate', function()
	local vehPlateNum = GetVehicleNumberPlateText(targetVeh)
	if plate == vehPlateNum or plate == "" then
		TriggerEvent('radio')
		ShowNotification("~b~NYPD Database: ~w~\nRecherche ~o~" .. vehPlateNum .. "~w~." )
		Wait(2000)
		ShowNotification("~w~Reg. Proprio : ~y~" .. regOwner .. "~w~\nReg. Année: ~y~" .. regYear .. "~w~\nEtat : ~y~" .. flags)
	else
		TriggerEvent('radio')
		ShowNotification("~o~NYPD Database: ~w~\nRunning ~o~" .. plate .. "~w~." )
		Wait(2000)
		TriggerEvent('getInfo')
		ShowNotification("~w~Reg. Proprio : ~y~" .. rregOwner .. "~w~\nReg. Année: ~y~" .. rregYear .. "~w~\nEtat : ~y~" .. rflags)
	end
end)

RegisterNetEvent('rpf:runid')
AddEventHandler('rpf:runid', function()
	if name == driverName or name == "" then
		if name == nil and driverQuestioned == false then
			ShowNotification("~r~Vous devez demander, avant sont identiter au ~o~Chauffeur !" )
		else
			TriggerEvent('radio')
			ShowNotification("~b~NYPD Database: ~w~\nRecherche ~o~" .. driverName .. "~w~." )
			Wait(2000)
...
RegisterNetEvent('rpf:search')
AddEventHandler('rpf:search', function()
--RegisterCommand("search", function()
	...
	if DoesEntityExist(searchVeh) or DoesEntityExist(searchPed) then
		...
		if DoesEntityExist(searchVeh) and IsEntityAVehicle(searchVeh) then
			ShowNotification("~b~Recherche~w~ sur le véhicule...")
			...
			ShowNotification("~b~Recherche~w~ sur le subjet...")
			Wait(3000)
		end
		if search == true then
		ShowNotification("~w~Trouver ~r~"..illegalItems[math.random(#illegalItems)])
		...
		ShowNotification("~w~Trouver ~g~rien de special.")
		end
	else
		ShowNotification("~r~Vous devez regarder le civile !")
	end
end)

RegisterNetEvent('rpf:breath')
AddEventHandler('rpf:breath', function()
--RegisterCommand("breath", function()
	...
	if DoesEntityExist(breathPed) then
		...
		TriggerEvent('search')
		ShowNotification("~w~Resulta ~b~Alcolemie~w~ test...")
		Wait(3000)
		if breathNum >= 8 then
		ShowNotification("~b~BAC~w~ Niveau: ~r~0." .. breathNum)
		else
		ShowNotification("~b~BAC~w~ Niveau: ~g~0." .. breathNum)
		end
	end
...
RegisterNetEvent('rpf:drug')
AddEventHandler('rpf:drug', function()
...
--distanceToVeh = GetDistanceBetweenCoords(GetEntityCoords(player), GetEntityCoords(stoppedDriver))
	...
	if DoesEntityExist(drugPed) then
		...
		TaskTurnPedToFaceEntity(drugPed, player, 6000)
		TriggerEvent('search')
		ShowNotification("~w~Recherche a ~b~Drogue~w~ test...")
		Wait(3000)
		ShowNotification("~w~Resultas:\n~b~  Cannabis~w~: " .. cannabis .. "\n~b~  Cocaine~w~: " .. cocaine)
	...
	if DoesEntityExist(drugVeh) then
		...
		if DoesEntityExist(drugDriver) then
			TriggerEvent('search')
			ShowNotification("~w~Recherche a ~b~Drugue~w~ test...")
			Wait(3000)
			ShowNotification("~w~Resultas:\n~b~  Cannabis~w~: " .. cannabis .. "\n~b~  Cocaine~w~: " .. cocaine)
		end
...
RegisterNetEvent('rpf:askid')
AddEventHandler('rpf:askid', function()
...
RegisterNetEvent('rpf:exit')
AddEventHandler('rpf:exit', function()
	if stopped then
		Citizen.CreateThread(function()
			if speech == "Normal" then
...
RegisterNetEvent('rpf:mount')
AddEventHandler('rpf:mount', function()
	if stopped then
...
RegisterNetEvent('rpf:release')
AddEventHandler('rpf:release', function()
	if stopped then
	if speech == "Normal" then
